...
const leadershipContent = {
  ...
  description:
    "Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo optio saepe ad soluta culpa reprehenderit fuga et nemo?",
  leaders: [
    {
...
const Leadership = ({ classNames }) => {
  ...
  return (
    <section
      className={`${cn(classNames)} pt-16 pb-16 md:pb-32 relative z-[1]`}
    >
      <div className="max-w-[1200px] mx-auto px-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-10">
          {data.leaders.map((leader, index) => {
            ...
            return (
              <motion.div
                key={leader.link} // ✅ burada key eklendi
                initial={{ opacity: 0, y: 20 }}
                whileInView={{
                  opacity: 1,
                  y: 0,
              ...
              >
                <Link href={leader.link}>
                  ...
                  />
                  <span className="block text-center uppercase text-[11px] tracking-widest">
                    {leader.title}
                  </span>
                  <h3 className="text-xl font-serif text-dark mb-1">
                    {leader.name}
                  </h3>
                  <span className="text-center text-base text-dark/50">
                    {leader.expertise}
                  </span>
                </Link>
              </motion.div>
